...
if __name__ == '__main__':
	...
	parser.add_option("-c","--carf",type="eng_float",\
			default=400E6,help="Set carrier frequency.")
	...
	parser.add_option("-x","--fft_len",type="eng_float",\
			default=1024,help="Set carrier frequency.")
	...
	usrp_source.set_gain(options.gain, 0)
        usrp_source.set_center_freq(uhd.tune_request(options.carf, 0, \                   
               dsp_freq_policy = uhd.tune_request.POLICY_MANUAL,\               
               rf_freq_policy = uhd.tune_request.POLICY_MANUAL), 0) 
	#usrp_source.set_auto_dc_offset(0, 0)
	#usrp_source.set_dc_offset(0+0j, 0)
	...
	countMax = 1
	freq_increment = 10E6
	#num_sample = 1E3*options.nstream*options.fft_len
	...
	while True:
		...
		os.rename("fft_average",\
			"fft_average"+str(int(options.carf))+"_"+ str(count))
		os.rename("fft_variance",\
			"fft_variance"+str(int(options.carf))+"_"+ str(count))
		os.rename("time_domain",\
			"time_domain"+str(int(options.carf))+"_"+ str(count))
		tb = top_block(options, usrp_source)
		#time.sleep(5)
		if count > countMax-1:
			count = 0
			print "Complete! Good job."
			options.carf = options.carf + freq_increment
			usrp_source.set_center_freq(options.carf, 0)
			tb = top_block(options, usrp_source)
