...
if __name__ == '__main__':
	...
	while True:
		...
		os.rename("fft_average",\
			"fft_average"+str(int(options.nstream))+"_"+ str(count))
		os.rename("fft_variance",\
			"fft_variance"+str(int(options.nstream))+"_"+ str(count))
		os.rename("time_domain",\
			"time_domain"+str(int(options.nstream))+"_"+ str(count))
		tb = top_block(options, usrp_source)
		#time.sleep(5)
		if count > countMax-1:
			count = 0
			print "Complete! Good job."
			options.nstream = options.nstream + nstream_increment
			tb = top_block(options, usrp_source)
			print "Printing out the noise estimation at " \
				+ str(options.nstream)	
			if options.nstream > 200:
				sys.exit(1)
