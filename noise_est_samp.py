...
if __name__ == '__main__':
	...
	parser.add_option("-s","--samp",type="eng_float",\
			default=20E6,help="Set carrier frequency.")
	...
	while True:
		...
		os.rename("fft_average",\
			"fft_average"+str(int(options.samp))+"_"+ str(count))
		os.rename("fft_variance",\
			"fft_variance"+str(int(options.samp))+"_"+ str(count))
		os.rename("time_domain",\
			"time_domain"+str(int(options.samp))+"_"+ str(count))
		tb = top_block(options, usrp_source)
		#time.sleep(5)
		if count > countMax-1:
			count = 0
			print "Complete! Good job."
			options.samp = options.samp/samp_decrement
			usrp_source.set_samp_rate(options.samp)
			tb = top_block(options, usrp_source)
			print "Printing out the noise estimation at " \
				+ str(options.samp) + "Hz"	
			if options.samp < 300E3:
				sys.exit(1)
